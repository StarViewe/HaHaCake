  ...
  <div>
    <el-form :model="form" ref="ruleFormRef" :rules="rules" label-width="auto" style="max-width: 600px">
      ...
      <el-form-item label="密码" prop="password">
        <el-input v-model="form.password" placeholder="请输入您的密码" show-password style="width: 240px" :prefix-icon="Lock"
          size="large"></el-input>
      </el-form-item>
      <el-form-item label="手机号" prop="phonenum">
        <el-input v-model="form.phone" style="width: 240px" placeholder="请输入您的手机号" :prefix-icon="Phone" maxlength="11"
          onkeyup="this.value=this.value.replace(/\D/g,'')" size="large"></el-input>
      </el-form-item>
      <el-form-item label="验证码" prop="capthca">
        <el-input v-model="form.capthca" placeholder="验证码" style="width: 125px" :prefix-icon="CaretRight" maxlength="4"
          size="large" onkeyup="this.value=this.value.replace(/\D/g,'')"></el-input>
        <el-button style="transform: translate(0px, 0); height: 48px; margin-left: 15px">
          获取验证码
        </el-button>
      </el-form-item>
      <el-form-item>
        <el-button @click="submit" class="reg-button">
          注册
        </el-button>
      </el-form-item>
    </el-form>
...
let submit = function doSubmit(vm: any) {
  let url = "http://101.37.83.158:8081/users/register";
  let data = {
  ...
  axios.post(url, data, {
    ...
    if (r.data.success) {
      ...
      localStorage.setItem('registeredPhonenum', form.phone);
      ElMessage.success({
        message: '注册成功',
        type: 'success',
        onClose: () => {
      ...
      vm.$message.error(r.data.message);
    }
  }).catch(error => {
    // 处理错误
    console.error(error);
